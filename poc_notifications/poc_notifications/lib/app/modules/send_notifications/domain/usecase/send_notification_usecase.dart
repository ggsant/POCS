...
class SendNotificationUseCaseImpl implements SendNotificationUseCase {
  ...
  Future<Either<NotificationsFailures, NotificationResult>> call(
    ...
    if (params.title.isEmpty) {
      return Left(
          const ValidationNotificationFailure('O titulo não pode ser vazio.'));
    } else if (params.body.isEmpty) {
      return Left(
          const ValidationNotificationFailure('O body não pode ser vazio.'));
    } else if (params.appId.isEmpty) {
      return Left(
          const ValidationNotificationFailure('O appId não pode ser vazio.'));
    }
