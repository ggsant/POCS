...
void main() {
  ...
  group('NotificationStore', () {
    storeTest<NotificationStore>(
      ...
      act: (store) => store.sendNotifications('title', 'body'),
    ...
    storeTest<NotificationStore>(
      ...
      build: () {
        when(() => usecase.call(any())).thenAnswer((_) async => Left(const DataSourceFailure('Error')));
        return NotificationStore(usecase);
      },
      act: (store) => store.sendNotifications('title', 'body'),
      expect: () => [tripleState, true, const DataSourceFailure('Error'), false],
      verify: (store) {
        verify(() => usecase.call(any())).called(1);
      },
    );
