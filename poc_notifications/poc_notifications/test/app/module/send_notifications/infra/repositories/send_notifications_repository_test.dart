...
class SendNotificationDataSouceImplMock extends Mock
    implements SendNotificationDataSouceImpl {}
...
void main() {
  ...
  final tNotificationModel = NotificationResult('');

  group('SendNotificationRepositoryImpl', () {
    test(
        'Should return NotificationModel when sendNotifications calls the datasource ',
        () async {
      //*arrange
      when(() => datasource.sendNotifications(const NotificationParamsEmpty()))
          .thenAnswer((_) async => tNotificationModel);
      //?act
      final response =
          await repository.sendNotifications(const NotificationParamsEmpty());
      //!assert
      expect(response.isRight(), true);
      expect(response, Right(tNotificationModel));
      verify(() =>
              datasource.sendNotifications(const NotificationParamsEmpty()))
          .called(1);
    });
    test(
        'should return a DataSourceFailure when the sendNotifications call to datasource is unsucessful - DIO',
        () async {
      //*arrange
      when(() => datasource.sendNotifications(const NotificationParamsEmpty()))
      ...
      final response =
          await repository.sendNotifications(const NotificationParamsEmpty());
      //!assert
      expect(response.fold(id, id), isA<DataSourceNotificationFailure>());
      verify(() =>
              datasource.sendNotifications(const NotificationParamsEmpty()))
          .called(1);
    });
